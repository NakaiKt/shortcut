...
export const commands: Command[] = [
  ...
  },
  {
    ...
    category: 'console',
    name: '環境変数を設定（一時的）',
    description: '現在のシェルセッションだけで有効な環境変数を設定します。ターミナルを閉じると消えます',
    windowsCommand: 'set {変数名}={値}',
    macCommand: 'export {変数名}={値}',
    options: [
      {
        id: 'powershell',
        windows: '$env:{変数名} = "{値}"',
        mac: 'export {変数名}="{値}"',
        description: 'Windowsは PowerShell での書き方。値に空白や記号を含む場合はダブルクォートで囲む',
      },
      {
        id: 'from-command',
        windows: 'for /f "delims=" %i in (\'{コマンド}\') do set {変数名}=%i',
        mac: 'export {変数名}=$({コマンド})',
        description: 'コマンドの実行結果を環境変数に格納する',
      },
      {
        id: 'one-shot',
        windows: 'set {変数名}={値} && {コマンド}',
        mac: '{変数名}={値} {コマンド}',
        description: '1つのコマンドの実行時だけ環境変数を渡す（Macはシェルに残らない）',
      },
      {
        id: 'dotenv',
        windows: 'for /f "usebackq tokens=*" %i in (".env") do set %i',
        mac: 'set -a && source .env && set +a',
        description: '.envファイルの内容をまとめて環境変数として読み込む',
      },
    ],
  },
  {
    ...
    category: 'console',
    name: '環境変数を永続化',
    description: 'ターミナルを閉じても残るようにユーザー環境変数として保存します',
    windowsCommand: 'setx {変数名} "{値}"',
    macCommand: 'echo \'export {変数名}="{値}"\' >> ~/.zshrc && source ~/.zshrc',
    options: [
      {
        id: 'bash',
        windows: 'setx {変数名} "{値}" /M',
        mac: 'echo \'export {変数名}="{値}"\' >> ~/.bash_profile && source ~/.bash_profile',
        description: 'Windowsはシステム全体に設定（管理者権限が必要）。Macはbashを使っている場合の設定先',
      },
      {
        id: 'reload',
        windows: '（新しいターミナルを開くと反映）',
        mac: 'source ~/.zshrc',
        description: 'setxは実行中のターミナルには反映されないため開き直す。Macは設定ファイルを再読み込みする',
      },
    ],
  },
  {
    ...
    category: 'console',
    name: '環境変数の一覧表示',
    description: '現在のシェルで有効な環境変数をすべて表示します',
    windowsCommand: 'set',
    macCommand: 'printenv',
    options: [
      {
        id: 'powershell',
        windows: 'Get-ChildItem Env:',
        mac: 'env',
        description: 'Windowsは PowerShell での書き方。Macは env でも同じ一覧を取得できる',
      },
      {
        id: 'sorted',
        windows: 'set | sort',
        mac: 'printenv | sort',
        description: '変数名でソートして表示する',
      },
      {
        id: 'filter',
        windows: 'set | findstr {キーワード}',
        mac: 'printenv | grep {キーワード}',
        description: 'キーワードを含む環境変数だけに絞り込む（例：AWS、NODE）',
      },
    ],
  },
  {
    ...
    category: 'console',
    name: '特定の環境変数を確認',
    description: '指定した環境変数の値だけを表示します',
    windowsCommand: 'echo %{変数名}%',
    macCommand: 'printenv {変数名}',
    options: [
      {
        id: 'echo',
        windows: 'echo $env:{変数名}',
        mac: 'echo ${変数名}',
        description: 'Windowsは PowerShell での書き方。Macは echo でも値を確認できる',
      },
      {
        id: 'exists',
        windows: 'if defined {変数名} (echo あり) else (echo なし)',
        mac: '[ -n "${変数名}" ] && echo あり || echo なし',
        description: '環境変数が設定されているかどうかだけを判定する',
      },
      {
        id: 'path',
        windows: 'echo %PATH%',
        mac: 'echo $PATH | tr \':\' \'\\n\'',
        description: 'PATHの確認（Macは1行1パスに分けて見やすく表示）',
      },
    ],
  },
  {
    ...
    category: 'console',
    name: '環境変数を削除',
    description: '現在のシェルセッションから環境変数を削除します',
    windowsCommand: 'set {変数名}=',
    macCommand: 'unset {変数名}',
    options: [
      {
        id: 'powershell',
        windows: 'Remove-Item Env:\\{変数名}',
        mac: 'unset {変数名1} {変数名2}',
        description: 'Windowsは PowerShell での書き方。Macは複数の変数をまとめて削除できる',
      },
      {
        id: 'permanent',
        windows: 'reg delete HKCU\\Environment /F /V {変数名}',
        mac: 'sed -i \'\' \'/^export {変数名}=/d\' ~/.zshrc && source ~/.zshrc',
        description: '永続化した環境変数を削除する（setxや設定ファイルで登録したもの）。Windowsは再ログインで反映',
      },
    ],
  },
  {
